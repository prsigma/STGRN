...
class InferenceNet(nn.Module):
    ...
    def forward(self, x, adj, temperature=1.0):
        logits, prob, y = self.qyx(x.squeeze(2), temperature)   #根据x得到y分布
        mu, logvar = self.qzxy(x, y)     #结合输入的嵌入向量 x 和通过 Gumbel-Softmax 分布获取的类别分布 y，来推断潜在变量 z 的分布
        mu_ori = mu
...
class VAE_EAD(nn.Module):
    ...
    def forward(self, x, dropout_mask, temperature=1.0, opt=None, ):
        ...
        adj_A_t = self._one_minus_A_t(self.adj_A * mask)        #计算I-W的转置
